    ...
    <body>
                    ...
                    <div class="col-md-10 col-lg-8 col-xl-7">
                        <div class="site-heading">
                            <h1>
                                ...
                                $query = "SELECT cats_id, name FROM categories WHERE slug = '$tags'";
                                $result = mysqli_query($conn, $query);
                            
                                if ($row = mysqli_fetch_assoc($result))
                                {
                                    $c_cats_id = $row['cats_id'];
                                    $name = $row['name'];
                                    echo $name;
                                }
                                ?>
        ...
        <div class="container px-4 px-lg-5">
            <div class="row gx-4 gx-lg-5 justify-content-center">
                <div class="col-md-10 col-lg-8 col-xl-7">
                    <?php
                    $query =  "SELECT id, title, subtitle, author, date_posted, cats_id from posts";
                    $result = mysqli_query($conn,$query);
                    ...
                    $count = 0;
                    {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $id = $row['id'];
                            $cats_id = unserialize($row['cats_id']);
                            if (in_array($c_cats_id,$cats_id))
                            {
                                $count = $count + 1;
                        ?>
                        ...
                        if ($count == 0)
                        {
                            ?>
                                <div class="post-preview">
                                    <h2 class="post-title">
                                        Currently having no post! Coming soon!
                                    </h2>
                    ...
                    ?>
                    <!-- Pager-->
                    <div class="d-flex justify-content-end mb-4"><a class="btn btn-primary text-uppercase" href="allposts.php">Other Posts →</a></div>
                </div>
            </div>
        </div>
        <!-- Footer-->
